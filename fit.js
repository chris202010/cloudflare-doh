...
- Distinguishes between “not configured” (silent fallback) and
- “configured but invalid JSON” (logged warning + fallback).
...
- @returns {Object} Path mappings configuration
  */
  ...
  // Already an object (bound via wrangler.toml [vars])
  if (typeof env.DOMAIN_MAPPINGS !== ‘string’) {
  return env.DOMAIN_MAPPINGS;
  }
  // String value: attempt JSON parse
  try {
  return JSON.parse(env.DOMAIN_MAPPINGS);
  } catch (error) {
  // FIX (中): distinguish config error from missing config; log clearly
  console.error(‘DOMAIN_MAPPINGS JSON 解析失败，已回退至默认配置。错误信息:’, error.message);
  }
  }
...
```
// Serve homepage for root and /index.html
...
// FIX (高): Use exact match or prefix + '/' to prevent /googleother matching /google
const pathPrefix = Object.keys(pathMappings).find(
...
if (pathPrefix) {
	...
	// FIX (高): Strip the original Host header so the upstream server receives
	// its own hostname, not the Worker's domain. Let fetch set Host automatically.
	const newHeaders = new Headers(request.headers);
	newHeaders.delete('host');

	// FIX (高): Do not forward a body for GET / HEAD requests (RFC 7231).
	...
	// FIX (中): Catch upstream network errors and return a proper 502 response
	// instead of letting an unhandled exception surface to the caller.
	try {
		return await fetch(newRequest);
	} catch (err) {
		console.error(`上游请求失败 [${newUrl}]:`, err);
		return new Response('Bad Gateway: 上游 DoH 服务暂时不可用，请稍后重试。', {
			status: 502,
...
// No matching mapping → serve homepage rather than a bare 404
return serveHomepage();
```
